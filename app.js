...
function getEffectiveDate(date) {
  // If this date has a swap, treat it as the swapped date for gym/meal purposes
  return (S.daySwaps && S.daySwaps[date]) ? S.daySwaps[date] : date;
}
function isGymDay(date) {
  const effective = getEffectiveDate(date);
  const d = new Date(effective+'T12:00:00');
  return S.gymDays.includes(d.getDay());
}
...
function nav(id, btn) {
  ...
  if(id==='settings') { loadSettingsForm(); renderProfileDisplay(); renderGymDaysCard(); renderExLibrary(); renderSwapList(); }
}
...
function renderDash() {
  ...
  // Show active swaps banner
  const banner = document.getElementById('activeSwapsBanner');
  if(banner && S.daySwaps) {
    const todaySwap = S.daySwaps[today()];
    if(todaySwap) {
      const d = new Date(todaySwap+'T12:00:00');
      banner.style.display = 'block';
      banner.innerHTML = '<div style="background:rgba(255,123,44,0.08);border:1px solid rgba(255,123,44,0.25);border-radius:10px;padding:10px 14px;font-size:12px;color:var(--orange);display:flex;align-items:center;justify-content:space-between">'
        +'<span>⇄ Today swapped with <strong>'+DAYS[d.getDay()]+' '+d.getDate()+' '+MONTHS[d.getMonth()]+'</strong></span>'
        +'<button onclick="removeSwap(today())" style="background:none;border:none;color:var(--muted);cursor:pointer;font-size:14px">✕</button>'
        +'</div>';
    } else {
...
function getPresetMeals(forDate) {
  ...
  const gym = isGymDay(date); // respects day swaps
  if(S.mealPlan) {
...
function renderWorkoutContent() {
  ...
  const gym = isGymDay(activeWDate);
...
function renderWeeklyPlanner() {
  ...
  grid.innerHTML = weekDates.map(date => {
    ...
    const swappedWith = S.daySwaps && S.daySwaps[date];
    const swapLabel = swappedWith ? (' <span class=\"swap-indicator\">⇄ swapped</span>') : '';
    ...
    return `<div class="planner-day ${isToday?'today':''} ${gym?'gym-day':'rest-day'}" onclick="togglePlannerDay('${date}')">
      ...
      ${swappedWith?'<div style="font-size:8px;color:var(--orange);text-align:center;margin-top:2px">⇄ swapped</div>':''}
...
// ═══════════════════════════════════════
/*
  Day swaps redirect a date to behave like another date.
  e.g. S.daySwaps['2025-06-04'] = '2025-06-05'
  means June 4 shows rest-day meals + no workout (because June 5 is a rest day)
  and June 5 gets gym day treatment.
  
  Supports:
  - Single day swap (today ↔ any day)
  - Week-level gym day overrides (temporary gym day changes for current week)
*/

let swapSourceDate = null; // date being swapped FROM

function openDaySwap(fromDate) {
  swapSourceDate = fromDate || today();
  const el = document.getElementById('modalDaySwap');
  if(!el) return;

  // Build the date options — next 14 days
  const picker = document.getElementById('swapDatePicker');
  picker.innerHTML = '';
  const now = new Date();
  for(let i=-7; i<=14; i++) {
    const d = new Date(now); d.setDate(now.getDate()+i);
    const key = d.toISOString().slice(0,10);
    if(key === swapSourceDate) continue;
    const gym = S.gymDays.includes(d.getDay());
    const hasSwap = S.daySwaps && S.daySwaps[key];
    const label = i===0?'Today ('+DAYS[d.getDay()]+')'
      :i===-1?'Yesterday ('+DAYS[d.getDay()]+')'
      :i===1?'Tomorrow ('+DAYS[d.getDay()]+')'
      :DAYS[d.getDay()]+' '+d.getDate()+' '+MONTHS[d.getMonth()];
    const div = document.createElement('div');
    div.className = 'swap-option';
    div.innerHTML = `
      <div class="swap-option-left">
        <div class="swap-option-label">${label}</div>
        <div class="swap-option-type ${gym?'gym':'rest'}">${gym?'💪 Gym Day':'🛌 Rest Day'}${hasSwap?' (already swapped)':''}</div>
      </div>
      <button class="btn btn-sm" onclick="applySwap('${swapSourceDate}','${key}')">Swap</button>
    `;
...
function applySwap(fromDate, toDate) {
  if(!S.daySwaps) S.daySwaps = {};

  // Bidirectional swap: fromDate acts like toDate, toDate acts like fromDate
  S.daySwaps[fromDate] = toDate;
  S.daySwaps[toDate] = fromDate;

  save();
  closeModal('modalDaySwap');
  renderSwapList();
  renderDash();
  renderWorkout();
...
function removeSwap(date) {
  if(!S.daySwaps) return;
  const paired = S.daySwaps[date];
  delete S.daySwaps[date];
  if(paired && S.daySwaps[paired] === date) delete S.daySwaps[paired];
  save();
  renderSwapList();
  renderDash();
  renderWorkout();
...
function renderSwapList() {
  ['activeSwapsList','settingsSwapList'].forEach(id => {
  const el = document.getElementById(id);
  if(!el) return;
  if(!S.daySwaps || !Object.keys(S.daySwaps).length) {
    el.innerHTML = '<div style="font-size:12px;color:var(--muted);padding:8px 0">No active swaps</div>';
    return;
  }
  // Show unique pairs only
  const shown = new Set();
  let html = '';
  Object.entries(S.daySwaps).sort().forEach(([from, to]) => {
    const pairKey = [from,to].sort().join('|');
    if(shown.has(pairKey)) return;
    shown.add(pairKey);
    const df = new Date(from+'T12:00:00');
    const dt = new Date(to+'T12:00:00');
    html += `<div class="swap-active-row">
      <div class="swap-active-info">
        <span style="color:var(--accent)">${DAYS[df.getDay()]} ${df.getDate()} ${MONTHS[df.getMonth()]}</span>
        <span style="color:var(--muted);font-size:11px;margin:0 6px">↔</span>
        <span style="color:var(--orange)">${DAYS[dt.getDay()]} ${dt.getDate()} ${MONTHS[dt.getMonth()]}</span>
      </div>
      <button class="btn-ghost" style="padding:4px 10px;font-size:11px" onclick="removeSwap('${from}')">✕ Remove</button>
    </div>`;
  });
  el.innerHTML = html || '<div style="font-size:12px;color:var(--muted);padding:8px 0">No active swaps</div>';
  }); // end forEach
}

function clearAllSwaps() {
  if(!confirm('Remove all day swaps?')) return;
  S.daySwaps = {};
  save();
  renderSwapList();
  renderDash();
  renderWorkout();
  showToast('All swaps cleared');
}
